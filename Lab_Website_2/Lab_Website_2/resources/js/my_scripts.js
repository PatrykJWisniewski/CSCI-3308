...
*/
let players = [{name:"John Doe", img: "../resources/img/player1.jpg", alt:"Image of Player 1", year:"Sophomore", major:"Art", games_played: 23, pass_yards: 435, rushing_yards: 200, receiving_yards: 88},
...
{
	let table = document.getElementById("stats_table");//Retrieve our table element
	let row_counter;//Keeps track of our row index
	let buffwins = 0;
	let bufflosses = 0;

	for(row_counter = 2; row_counter < table.rows.length; row_counter++)
	{//Outer for loop iterates over each row
		if(parseInt(table.rows[row_counter].cells[2].innerText) > parseInt(table.rows[row_counter].cells[3].innerText))
		{
		...
		{
			table.rows[row_counter].cells[4].innerHTML = table.rows[row_counter].cells[1].innerText;
			bufflosses++;
		}
	}

	document.getElementById("wins").innerText = buffwins.toString();
	document.getElementById("losses").innerText = bufflosses.toString();
